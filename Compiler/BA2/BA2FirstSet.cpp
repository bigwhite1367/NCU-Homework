...
int main(int argc, char *argv[])
{
	...
	{
		if(s == "END_OF_GRAMMAR" || s == "END_OF_GRAMMAR ")
		{
			break;
		}
		sets[s.substr(0, 1)] = "";
...
*/
	...
	for(uint counter = 0 ; counter<noTerminal.size(); counter++)
	{
		...
		reverse(sets[tmp].begin(), sets[tmp].end());
		removeDuplicate(sets[tmp]);
		//cout<<tmp<<" "<<sets[tmp]<<endl;
	}
	sort(noTerminal.begin(),noTerminal.end());
	for(uint counter = noTerminal.size() ; counter > 0; counter--)
	{
		string tmp = noTerminal.substr(counter-1,1);
		cout<<tmp<<" "<<sets[tmp]<<endl;
	}
...
bool findFirst(string tmp, string current) //tmp 代表這次進入的NonT  current代表這一大圈的NonT
{
	...
	bool flag=false;
	for(uint incounter = 0 ; incounter<rules.size() ; incounter++) // 走過每一條rulecout<<rules[incounter].substr(0,1)<<endl;
		if(tmp == rules[incounter].substr(0,1)) //當走到tmp時
		{
			//cout<<current<<" "<<sets[current]<<endl;	
			for(uint ptr = 1; ptr < rules[incounter].size() ; ptr++) //走過該rule
			{
				if(((isupper(rules[incounter][ptr])) || rules[incounter].substr(ptr,1) =="$")&& rules[incounter].substr(ptr,1) != ";") // 當是T且不是;時
				{
					//cout<<current<<" "<<sets[current]<<endl;
